...
3. 唔使安裝、唔使伺服器。資料存在本機瀏覽器。可用「過檔」出示／掃入 QR（狀態、分數、錯因、用時、操卷日），或匯出／匯入 JSON（匯出只含當前學生；匯入可一次多個檔，含筆記）
...
- 過檔 QR：狀態＋分數＋錯因＋用時＋操卷日；掃入後可合併／覆蓋／新增。JSON 仍作完整備份（含筆記）
- 年份掣「✓」＝該年格已填齊；「分」＝已填卷面分。操卷日人手填，有日期先出「清除日期」；用時由計時暫停後批准寫入，可「清除用時」
- 能力頁「強弱摘要」撳先開；打氣跟圖，淺軸未穩唔催深課題
- 雷達卷一撳課題名會去進度該課題；「顯示全部題」清課題／狀態篩。8 軸課題對照只列出該卷有出過嘅課題
- 摘錄可開「明返」（曾經一般／唔識、而家已掌握；同一綠色、格角一點）。JSON 會帶明返；QR 暫不另帶，合併時保留本機標記
- 撳摘錄課題條，下面出該課題未做建議（淺→深，最多 5 題；略過今日先操完嗰年）
